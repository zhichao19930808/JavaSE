...
public class FileTest {
    ...
    public static void main(String[] args) {
        File file = new File("test");//通过将给定路径名字符串转换为抽象路径名来创建一个新 File 实例。
        ...
        System.out.println(file.getParent());//null  返回此抽象路径名父目录的路径名字符串；如果此路径名没有指定父目录，则返回 null。
        System.out.println(file.getParentFile());//null  返回此抽象路径名父目录的抽象路径名；如果此路径名没有指定父目录，则返回 null。

        System.out.println(new Date(file.lastModified()));// date 日期  上次更改的时间
